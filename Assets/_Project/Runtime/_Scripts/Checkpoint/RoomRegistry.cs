...
public class RoomRegistry : MonoBehaviour
{
    ...
    Light clearLight;
    ParticleSystem[] lightEffects;
    List<Light> lightEffectsPointLight = new();
    ...
    private void Awake()
    {
        ...
        if (clearLight != null) clearLight.enabled = false;
        UpdateLightState();

        lightEffects = GetComponentsInChildren<ParticleSystem>();

        for (int i = 0; i < lightEffects.Length; i++)
        {
            var subLight = lightEffects[i].GetComponentInChildren<Light>();
            if (subLight != null)
            {
                lightEffectsPointLight.Add(subLight);
    ...
    private void UpdateLightState()
    {
        ...
        if(enemies.Count == 0)
        {
            if (clearLight != null)
            {
                ...
                for (int i = 0; i < lightEffectsPointLight.Count;  i++)
                {
                    lightEffectsPointLight[i].enabled = true;
                }
            }
        }
    }
