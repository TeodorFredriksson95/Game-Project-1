...
public class Enemy : MonoBehaviour, IDamageable, IEnemyReset, IInteractable
{
    ...
    void Awake()
	{
        ...
        if (Mesh == null)
            Mesh = GetComponentInChildren<Animator>()?.gameObject;
	...
	void Update()
	{
        ...
        // Enemy attack range
        #region Teo wants to learn math. Sue me
        float offsetDistance = 3f;
		// Get initial direction from player towards enemy
		Vector3 direction = (transform.position - target.position);
		// Square directional values
		float squaredX = direction.x * direction.x;
		float squaredY = direction.y * direction.y;
		float squaredZ = direction.z * direction.z;
		// Get length of initial direction
		float dirLen = Mathf.Sqrt(squaredX + squaredY + squaredZ);
		// Normalize directional values
		float normalizedX = direction.x / dirLen;
		float normalizedY = direction.y / dirLen;
		float normalizedZ = direction.z / dirLen;
		// Define normalized vector
		Vector3 normalizedDir = new Vector3(normalizedX, normalizedY, normalizedZ);

		// Extend the direction in a circle based on the unit vector and offset relative to players position
		Vector3 desired = target.position + normalizedDir * offsetDistance;
        #endregion
	...
	void Death()
	{
		...
		var renderers = Array.Empty<Renderer>();
		...
		switch (type)
		{
			case EnemyType.Octopus:
				index = 0;
				renderers = transform.GetChild(index).GetComponentsInChildren<Renderer>();
				deathMaterial = octopusDeathMaterial;
				break;

			case EnemyType.Banshee:
				index = 1;
				renderers = transform.GetChild(index).GetComponentsInChildren<Renderer>();
				deathMaterial = watcherDeathMaterial; // assuming same mat as before — adjust if needed
				break;

			case EnemyType.Octopus2:
				index = 2;
				renderers = transform.GetChild(index).GetComponentsInChildren<Renderer>();
				deathMaterial = octopusDeathMaterial; // you'll need to define this if not done yet
				break;

			default:
				Debug.LogWarning($"Unhandled enemy type: {type}");
				break;
		}
