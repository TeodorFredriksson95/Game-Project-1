...
public class SpikeTrap : MonoBehaviour
{
	...
	void Static()
	{
		if (@static)
		{
			spikeMesh.transform.DOLocalMoveZ(0.02f, 0.2f).SetEase(Ease.InExpo);
			trapSFX.Play();
	...
	IEnumerator Periodic()
	{
		while (periodic)
		{
			...
			sequence.Append(spikeMesh.transform.DOLocalMoveZ(0.02f, 0.2f).SetEase(Ease.InExpo));
			sequence.AppendCallback(() => trapSFX.Play());
			sequence.AppendInterval(0.2f);
			sequence.Append(spikeMesh.transform.DOLocalMoveZ(-0.036f, 0.2f).SetEase(Ease.OutExpo));
