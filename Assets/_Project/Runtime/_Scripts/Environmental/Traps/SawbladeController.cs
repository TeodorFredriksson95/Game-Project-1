...
public class SawbladeController : MonoBehaviour
{
    ...
    private bool removePlayerOnUpdate;

    private void OnEnable()
    {
        PlayerHealth.OnPlayerDied += RemovePlayerRef;
    }
    ...
    void Update()
    {
        if (removePlayerOnUpdate)
        {
            currentTargets.Remove(playerHealth.gameObject);
            removePlayerOnUpdate = false;
        }
    ...
    void HandleChildCollision(Collider collision, bool entered)
    {
        ...
        if (entered && !playerHealth.IsDead)
        {
