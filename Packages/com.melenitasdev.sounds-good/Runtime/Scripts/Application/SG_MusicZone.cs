...
namespace MelenitasDev.SoundsGood
{
    ...
    public partial class SG_MusicZone : MonoBehaviour
    {
        void Awake()
        {
            ...
            {
                Debug.LogError($"{name}: No Main Camera found for SG_MusicZone.");
                enabled = false;
                return;
            }
            ...
            if (playerMode == PlayerMode.Music)
            {
                ...
                {
                    Debug.LogError($"{name}: SG_MusicZone in Music mode has no tracks assigned.");
                    enabled = false;
                    return;
                }
            ...
            else if (playerMode == PlayerMode.Playlist)
            {
                if (tracks == null || tracks.Count == 0)
                {
                    Debug.LogError($"{name}: SG_MusicZone in Playlist mode has no tracks assigned.");
                    enabled = false;
                    return;
                }
            ...
            {
                if (dynamicTracks == null || dynamicTracks.Count == 0)
                {
                    Debug.LogError($"{name}: SG_MusicZone in DynamicMusic mode has no dynamicTracks assigned.");
                    enabled = false;
                    return;
                }

                Track[] tracksArray = new Track[dynamicTracks.Count];
                int i = 0;
                foreach (TrackInfo dynamicTrack in dynamicTracks)
                {
                    tracksArray[i] = dynamicTrack.track;
                    i++;
                }

                dynamicMusic = new DynamicMusic(tracksArray);
                dynamicMusic
                    .SetLoop(loop)
                    .SetSpatialSound(false)
                    .SetPosition(transform.position)
